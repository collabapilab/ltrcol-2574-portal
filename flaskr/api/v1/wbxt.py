...
wbx_bot_api = WebexSimpleApi(tokens=WBX_BOT_ACCESS_TOKEN)
...
def create_webhooks():
    ...
    for resource_type in ['messages', 'attachmentActions']:
        webhook = wbx_bot_api.webhook.create(
            ...
            target_url=WBX_WEBHOOK_URL
        )
        log.info(f'Webhook "{WBX_WEBHOOK_NAME}" for "{webhook.resource}" created. Callback URL="{WBX_WEBHOOK_URL}".')
