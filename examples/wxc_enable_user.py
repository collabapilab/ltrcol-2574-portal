...
# Replace with your temporary personal access token from your user admin account (from developer.webex.com)
personal_access_token = '___PASTE_YOUR_PERSONAL_ACCESS_TOKEN_HERE___'

# Create an instance of the WebexSimpleApi with your personal access token
api = WebexSimpleApi(tokens=personal_access_token)
...
for webex_user in webex_user_list:
    log.info(f'User information found for {webex_user.display_name}:')
    log.info(webex_user.json(indent=4))
    ...
    log.info(webex_user_det.json(indent=4))
...
for wxc_license in webex_license_list:
    ...
    if wxc_license.name == 'On-Prem UCM Calling':
...
log.info(modified_webex_user.json(indent=4))
